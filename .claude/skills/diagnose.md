...
1. **Venice key test** — call `node --env-file=.env --import tsx -e "import { loadPrivyConfig, loadSignerFromPrivy } from './harness/safety/wallet.ts'; import { withVeniceKey } from './platform/venice-auth.ts'; const s = await loadSignerFromPrivy(loadPrivyConfig()); await withVeniceKey(s, async k => { console.log('venice_ok key_prefix=' + k.slice(0,8)); })"` — if it throws, capture the error.
