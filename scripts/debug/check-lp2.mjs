...
const BALANCE_ABI = [{ name: 'balanceOf', type: 'function', stateMutability: 'view',
...
const diemAmount = await client.readContract({ address: DIEM, abi: BALANCE_ABI, functionName: 'balanceOf', args: [WALLET] });
const deadline = BigInt(Math.floor(Date.now() / 1000) + 600);

console.log('Tick:', currentTick, '| Range:', tickLower, '-', tickUpper);
console.log('DIEM to LP:', formatUnits(diemAmount, 18));

try {
  const result = await client.simulateContract({
    address: NFPM, abi: NFPM_MINT_ABI, functionName: 'mint',
    account: WALLET,
    args: [{
  ...
  console.log('SUCCESS: tokenId=%s liquidity=%s amount1=%s', result.result[0], result.result[1], formatUnits(result.result[3], 18));
} catch(e) {
